// DCC Function Decoder AYA002-2 or AYA002-4 for DS-DCC decode
// By yaasan
...
void PantaSparkEffect_Control2(){
  ...
  if(State_O[4][1]== 0){ // PantaSpark O2:OFF
    state = 0; 
    adr = 0;
    timeUp = 0;
    pwmRef = 0;
...
//       OCR1A=0; 
  ...
  switch(state){
    case 0:
      if(State_O[4][1] == 1){ // PantaSpark O2:ON
        adr = 0;
        timeUp = 0;
        pwmRef = 0;
        //               OCR1A有効   high出力          分周無し　
        TCCR1 = 1<<PWM1A | 2<<COM1A0 | 1<<CS10; 
        // analogWrite(O2, 0); // analogWriteは使えない 
        OCR1A = 0;
        state = 1;
        goto S20;     // 100ms待たずに再度ステートマシーンに掛ける
      }
      break;
    ...
    case 2: // 光るパターンを４種類からランダムに算出
      ...
      adr = 0; 
      state = 4;
      goto S20;   // 10ms待たずに再度ステートマシーンに掛ける
      break;
    ...
    case 5: // S05:時間カウント
    //analogWrite(O2, (unsigned char)pwmRef); // 0〜255

      OCR1A = (unsigned char)pwmRef; // 0〜255   
      timeUp--;
      if( timeUp <= 0 ){
        state = 4;                            // 次のコマンドへ
        if( ptn[adr][0]=='E'){                // シーケンス終了
